library(ggplot2)
getwd()
setwd("Output_path_of_CellphoneDB")
...
det_i_pairs=sapply(colnames(rec_mx), function(x){
  intr_pairs[str_detect(intr_pairs,x)]
})
sel_row<-unique(unlist(det_i_pairs))

dot_plot(selected_rows = sel_row,
         selected_columns = colnames(all_pval)[13:14],
         height = 8,
         width =length(sel_row)/4,
         filename = 'plot_R.pdf')
...
dot_plot = function(selected_rows = NULL,
  ...
  ggplot(plot.data,aes(y=clusters,x=pair)) +
